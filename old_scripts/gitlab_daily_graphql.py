#!/usr/bin/env python3
"""
gitlab_daily_graphql.py - Auto-generate Obsidian daily notes from GitLab data

Same output as gitlab_daily_note.py but uses GraphQL API (fewer HTTP calls)
and JSON state file (no more fragile markdown-parsing for diffs).

Requires the `glab` CLI to be installed and authenticated.

Usage:
    python gitlab_daily_graphql.py              # Generate today's note
    python gitlab_daily_graphql.py --keep       # Save a snapshot on first run of the day
    python gitlab_daily_graphql.py --init       # Create a config file with defaults
    python gitlab_daily_graphql.py --stdout     # Print to stdout instead of saving
"""
...
# ---------------------------------------------------------------------------
# Normalizers: GraphQL response -> REST-compatible dicts
# ---------------------------------------------------------------------------
...
def _normalize_mr(node):
    ...
    return {
        ...
        # Keep raw discussions for processing
...
class GitLabSync:
    ...
    def fetch_main(self):
        """Single GraphQL call: user info + todos + authored MRs + assigned MRs"""
        ...
        # MRs (deduplicate authored + assigned)
        mrs_by_key = {}
        ...
        # Process discussions and approvals from inline data
    ...
    def _process_approvals(self):
        print(f"Processing approvals...", file=sys.stderr)
        for mr in self.mrs:
    ...
    def build_issue_mr_links(self):
        """Fetch related MRs per issue (REST — GraphQL limits to 1 issue at a time)"""
        if not self.issues:
            return
        ...
        for issue in self.issues:
            ...
            for rel in related:
                url = rel.get("web_url", "")
                self.issue_all_mrs[issue["web_url"]].append({
                    "web_url": url,
                    "state": rel.get("state", ""),
                })
    ...
    def fetch_team_mrs(self):
        """Fetch team MRs — still REST (group-level GraphQL varies by GitLab version)"""
        if not self.username:
            return
    ...
    def compute_diff(self, date):
        ...
        prev_date_str = prev_state.get("date", "")
    ...
    def _is_on_hold(self, item):
        ...
        return any(pattern in full_ref or pattern in web_url for pattern in self.on_hold_patterns)
    ...
    def format_markdown(self, date, todos, diff=None):
        ...
        # Conflicts not already in act_now
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Generate Obsidian daily notes from GitLab data (GraphQL)",
